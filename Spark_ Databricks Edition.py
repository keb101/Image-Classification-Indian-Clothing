...
df_result1 = df_result.groupBy('occupation').agg(round(mean('is_divorced'),2).alias('divorced_rate'))
# do ordering
df_result2 = df_result1.orderBy(desc('divorced_rate'))
# show results
df_result.show(5), df_result1.show(5), df_result2.show(5)
...
df_result1 = df_result.groupBy('education').agg(round(mean('never_married'), 2).alias('bachelor_rate'))
df2 = df_result1.orderBy(desc('bachelor_rate'))
df2.show(1)
...
lrCvFeatureImportance = pd.DataFrame([(name, lrCvModel.bestModel.featureImportances[idx]) for idx, name in attrs],columns=['feature_name','feature_importance'])
...
transformed =  lrCvModel.transform(df)
